<?php
  include 'shared/variables.php';
  $pageTitle = 'Blog | Ilyce Glink';
  $bodyId = 'blog';
  include 'shared/head.php';
  include 'shared/header.php';
?>

<section class="main" role="main">
  <div class="inner">
    <div class="main__content blog-feed">
      <article>
        <figure class="post-image">
          <img src="http://placehold.it/240x240.jpg">
        </figure>
        <header>
          <p class="post-top__date"><time>January 1, 2014</time></p>
          <h2><a href="media-post.php">Media Entry Title</a></h2>
        </header>
        <div class="post-summary">
          <p>Lorem ipsum dolor sit <strong>bold text</strong> amet, consectetur adipiscing elit. Nam sit amet aliquam quam, eget dictum eros. Suspendisse hendrerit, lacus ut vestibulum commodo, nisi nulla hendrerit eros, et hendrerit urna metus quis elit. Vestibulum congue nunc et urna rhoncus ornare. Sed feugiat porta nibh eget sodales. Suspendisse potenti. Aliquam erat volutpat. Etiam ultricies luctus metus, eget egestas lacus volutpat in.</p>
          <p class="more"><a href="media-post.php">Keep reading</a></p>
        </div>
      </article>
      <article>
        <figure class="post-image">
          <img src="http://placehold.it/240x240.jpg">
        </figure>
        <header>
          <p class="post-top__date"><time>January 1, 2014</time></p>
          <h2><a href="media-post.php">Media Entry Title</a></h2>
        </header>
        <div class="post-summary">
          <p>Lorem ipsum dolor sit <strong>bold text</strong> amet, consectetur adipiscing elit. Nam sit amet aliquam quam, eget dictum eros. Suspendisse hendrerit, lacus ut vestibulum commodo, nisi nulla hendrerit eros, et hendrerit urna metus quis elit. Vestibulum congue nunc et urna rhoncus ornare. Sed feugiat porta nibh eget sodales. Suspendisse potenti. Aliquam erat volutpat. Etiam ultricies luctus metus, eget egestas lacus volutpat in.</p>
          <p class="more"><a href="media-post.php">Keep reading</a></p>
        </div>
      </article>
      <article>
        <figure class="post-image">
          <img src="http://placehold.it/240x240.jpg">
        </figure>
        <header>
          <p class="post-top__date"><time>January 1, 2014</time></p>
          <h2><a href="media-post.php">Media Entry Title</a></h2>
        </header>
        <div class="post-summary">
          <p>Lorem ipsum dolor sit <strong>bold text</strong> amet, consectetur adipiscing elit. Nam sit amet aliquam quam, eget dictum eros. Suspendisse hendrerit, lacus ut vestibulum commodo, nisi nulla hendrerit eros, et hendrerit urna metus quis elit. Vestibulum congue nunc et urna rhoncus ornare. Sed feugiat porta nibh eget sodales. Suspendisse potenti. Aliquam erat volutpat. Etiam ultricies luctus metus, eget egestas lacus volutpat in.</p>
          <p class="more"><a href="media-post.php">Keep reading</a></p>
        </div>
      </article>
      <article>
        <figure class="post-image">
          <img src="http://placehold.it/240x240.jpg">
        </figure>
        <header>
          <p class="post-top__date"><time>January 1, 2014</time></p>
          <h2><a href="media-post.php">Media Entry Title</a></h2>
        </header>
        <div class="post-summary">
          <p>Lorem ipsum dolor sit <strong>bold text</strong> amet, consectetur adipiscing elit. Nam sit amet aliquam quam, eget dictum eros. Suspendisse hendrerit, lacus ut vestibulum commodo, nisi nulla hendrerit eros, et hendrerit urna metus quis elit. Vestibulum congue nunc et urna rhoncus ornare. Sed feugiat porta nibh eget sodales. Suspendisse potenti. Aliquam erat volutpat. Etiam ultricies luctus metus, eget egestas lacus volutpat in.</p>
          <p class="more"><a href="media-post.php">Keep reading</a></p>
        </div>
      </article>
      <article>
        <figure class="post-image">
          <img src="http://placehold.it/240x240.jpg">
        </figure>
        <header>
          <p class="post-top__date"><time>January 1, 2014</time></p>
          <h2><a href="media-post.php">Media Entry Title</a></h2>
        </header>
        <div class="post-summary">
          <p>Lorem ipsum dolor sit <strong>bold text</strong> amet, consectetur adipiscing elit. Nam sit amet aliquam quam, eget dictum eros. Suspendisse hendrerit, lacus ut vestibulum commodo, nisi nulla hendrerit eros, et hendrerit urna metus quis elit. Vestibulum congue nunc et urna rhoncus ornare. Sed feugiat porta nibh eget sodales. Suspendisse potenti. Aliquam erat volutpat. Etiam ultricies luctus metus, eget egestas lacus volutpat in.</p>
          <p class="more"><a href="media-post.php">Keep reading</a></p>
        </div>
      </article>
      <footer class="pagination">
        <a class="prev" href="javascript:void(0)">Previous</a>
        <a class="next" href="javascript:void(0)">Next</a>
        <ol>
          <li><a href="javascript:void(0)">1</a></li>
          <li><a href="javascript:void(0)">2</a></li>
          <li><a href="javascript:void(0)">3</a></li>
          <li><a href="javascript:void(0)">4</a></li>
          <li><a href="javascript:void(0)">5</a></li>
        </ol>
      </footer>
    </div>
    <?php include 'shared/blog-sidebar.php'; ?>
  </div>  
</section>

<?php
  include 'shared/footer.php';
  include 'shared/foot.php';
?>